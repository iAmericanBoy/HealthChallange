...
class Challenge {
    ...
    init?(record: CKRecord) {
        guard let name = record[Challenge.nameKey] as? String,
            ...
            let weekGoals = record[Challenge.weekGoalsKey] as? [Goal],
            let monthlyGoal = record[Challenge.montlyGoalKey] as? Goal else {return nil}
        
        self.name = name
        self.monthlyGoal = monthlyGoal
        self.weekGoals = weekGoals
        self.startDay = startDay
...
extension CKRecord {
    ...
    convenience init(challenge: Challenge) {
        ...
        self.setValue(challenge.weekGoals, forKey: Challenge.weekGoalsKey)
        self.setValue(challenge.monthlyGoal!, forKey: Challenge.montlyGoalKey)
    }
}
